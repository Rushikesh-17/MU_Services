...
router.post("/vendor/register", upload.single("image"), function (req, res) {
  ...
  req
    .checkBody("username", "Username can only have  letters and numbers")
    .isAlphanumeric();
  ...
  req
    .checkBody(
      "password",
      "Password must be 8 - 20 characters long with one uppercase letter, one lowercase letter and one number"
    )
  ...
  req
    .checkBody("pincode", "Pincode is not valid")
    .isNumeric({ no_symbols: true });
  req
    .checkBody("mobile", "Mobile number can only have numbers")
    .isNumeric({ no_symbols: true });
  req
    .checkBody("mobile", "Mobile number is not valid")
    .matches("^[6-9][0-9]{9}$");
    ...
    if (errors) {
      ...
      Vendor.findById(req.params.id, async function (err, vendor) {
        if (err) {
          ...
          req.flash(
            "success",
            "Successfully updated details for " + vendor.username
          );
          res.redirect("/services");
        }
      });
        ...
        if (err) {
          ...
          if (res.locals.currentUser.isCustomer) {
            ...
            if (res.locals.currentUser.username != foundVendor.username) {
              ...
              res.render("vendor/v_show", {
                vendor: foundVendor,
                error:
                  "Your profile is not verified yet and will not be visible to others. Handyman team will contact you soon for verification.",
              });
            }
          }
        } else {
